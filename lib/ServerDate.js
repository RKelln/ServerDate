...
function mergeOptions() {
  var userOverrides = window.ServerDate;
  if (userOverrides) {
    for (var attrname in userOverrides) {
      if (config[attrname]) {
        config[attrname] = userOverrides[attrname];
      }
    }
  }
}
...
// clock and the server's clock.  Rather than make the change abruptly, we
// change our clock by adjusting it once per second by the amortizationRate.
ServerDate.amortizationRate = config.amortizationRate; // ms

// The exception to the above is if the difference between the clock and
// server's clock is too great (threshold set below).  If that's the case then
// we skip amortization and set the clock to match the server's clock
// immediately.
ServerDate.amortizationThreshold = config.amortizationThreshold; // ms

// Each syncronization requests multiple samples from the server and uses the
// one with least latency for determining the offset.
ServerDate.synchronizationRequestSamples = config.synchronizationRequestSamples;
...
// After the initial synchronization the two clocks may drift so we
// automatically synchronize again every synchronizationIntervalDelay.
ServerDate.synchronizationIntervalDelay = config.synchronizationIntervalDelay; // ms
...
function synchronize(cb) {
  ...
  function processSample(serverNow) {
    ...
    // low latency.
    if (iteration < ServerDate.synchronizationRequestSamples) {
      iteration++;
      requestSample();
    } else {
  ...
  ServerDate.syncronizationTimeout);
...
function log(message) {
    if (config.debug && console && console.log) { console.log("[ServerDate] " + message); }
}
