...
var
  ...
  synchronizing = false,
  lastUserClockTime,
  syncronizationTimeoutMultiplier = 1.0,
  synchronizationSuccessCount = 0,
...
function initConfig(userOverrides) {
  ...
  if (userOverrides) {
    Object.keys(userOverrides).forEach( function(prop) {
  ...
  Object.keys(ServerDate.config).forEach( function(prop) {
...
function synchronize(options) {
  ...
  function requestSample() {
    ...
    request.open("HEAD", (ServerDate.synchronizationURL || scriptURL) + "?noCache=" + Date.now());
  ...
  function finishSync(success, newTarget, oldTarget) {
    ...
    synchronizing = false;
    if (success) { synchronizationSuccessCount += 1; }
  ...
  syncTimeout = setTimeout(function () {
    ...
    if (syncronizationTimeoutMultiplier < 4.0) {
      syncronizationTimeoutMultiplier += 0.25;
    }
  },
  options.syncTimeoutDuration * syncronizationTimeoutMultiplier);
...
// Each second:
//  * check that the users clock hasn't been adjusted
//  * handle amortization
//  * sync if we've been been successful and we're not currently syncing
setInterval(function()
{
  handleUserClockChange();
  handleAmortization();
  // if we haven't set a target yet and we're not syncronizing, then try again
  if (synchronizationSuccessCount === 0 && !synchronizing) {
    log("No sync completed successfully, retrying sync");
    synchronize();
  }
}, 1000);
